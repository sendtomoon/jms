...
function viewInfo(id,title,height,width,fn,savefn){	
	...
	butts.push(html2);
	$("#"+id).removeClass('hide').dialog({resizable:false,modal:true,height:height,width:width,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h4></div>",title_html:true,buttons:butts});
...
function editInfo1(id,title,savefn,cancelfn){
	$("#"+id).removeClass('hide').dialog({resizable:false,modal:true,height:500,width:600,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
	$("#addDiv").removeClass('hide')
}
...
//			html+="<td style='display:none'>"+getSelectValue(item.feeType,feeTypes)+"</td>";
			...
			if(1==isHide){
				...
				html+="<td>";
				if(item.status!=2){
					html+="<a class='aBtnNoTD' onclick='splitAll(&apos;"+itemNum+"&apos;)' title='匹配' href='#'><i class='icon-edit color-blue bigger-120'></i></a>";		
				}
			}else if (0==isHide){
				html+="<td>"+JY.Object.notEmpty(item.description)+"</td>"
				html+="<td>";
			...
			+JY.Object.notEmpty(item.gMaterial)+"' gWeight='"+JY.Object.notEmpty(item.gWeight)+"' dWeight='"+JY.Object.notEmpty(item.dWeight)
	...
	if(JY.Object.notNull(nums)){
		...
		for(var i=0;i<numList.length;i++){
			...
			var item={mdCode:mdCode,mdtlCode:mdtlCode,gMaterial:gMaterial,gWeight:gWeight,dWeight:dWeight,dClarity:dClarity,dColor:dColor,
					...
					totalFee:totalFee,status:status,id:ids,description:description,beforeId:beforeId};
